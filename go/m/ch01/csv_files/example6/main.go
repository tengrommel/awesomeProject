...
func main() {
	...
	// Filter the dataframe again, but only select out the sepal_width and species column.
	versicolorDF = irisDF.Filter(filter).Select([]string{"sepal_width", "species"})
	fmt.Println(versicolorDF)

	// Filter and select the dataframe again, but only display the first three result.
