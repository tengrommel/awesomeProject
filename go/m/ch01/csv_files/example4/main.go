	...
	for {
		...
		for idx, value := range record {
			...
			if idx == 4 {
				...
				// And the string value to the CSVRecord.
				csvRecord.Species = value
				continue
			}
			// Otherwise, parse the value in the record as a float64.
			// floatValue will hold the parsed float value of the record for the numeric columns.
			var floatValue float64

			// If the value can not be parsed as a float, long and break the parsing loop.
